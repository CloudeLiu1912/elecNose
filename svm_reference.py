...
# 生成的数据是两个同心圆数据，每个不同的环代表不同的类，确保只有类-1或者1
...
sq_dists = tf.add(tf.subtract(dist,
                              tf.multiply(2., tf.matmul(x_data,
                                                        tf.transpose(x_data)))),
                  tf.transpose(dist))
...
for i in range(1000):
    ...
    # print('x_input.shape:', rand_x.shape)
    # print('y_input.shape:', rand_y.shape)
    # print('pred_input.shape:', rand_x.shape)
    # print('\n')
